...
def carregar_env_manual():
    """Lê .env nativamente sem travar no Android"""
    try:
...
# ==============================================================================
# 4. INTERFACE (V50 - FUNCIONALIDADE RESTAURADA)
# ==============================================================================
def main(page: ft.Page):
    ...
    try:
        ...
        page.bgcolor = "#0f172a"
        page.padding = 0
        page.scroll = ft.ScrollMode.AUTO # Scroll global para evitar tela azul
        ...
        def barra_simbolos():
            return ft.Container(content=ft.Row([
                ft.Icon(ft.icons.EXTENSION, color="#007ACC"), 
                ft.Icon(ft.icons.WB_SUNNY, color="#fbbf24"), 
                ft.Icon(ft.icons.ALL_INCLUSIVE, color="red")
        ...
        # 2. FERRAMENTAS (Com Checklist Completo)
        def tela_ferramentas():
            ...
            # Preço de Vida (Restaurado)
            tr = get_renda()/160 if get_renda()>0 else 0
            t_pv = ft.TextField(label="Preço do Item", prefix_text="R$ ", on_change=mascara_dinheiro, expand=True)
            ...
            def chat(e):
                r_chat.value="..."; page.update(); r_chat.value = chamar_gemini(t_chat.value) or "Erro"; page.update()
        ...
        # 3. CAÇADOR DE ASSINATURAS (RESTAURADO)
        def tela_assinaturas():
            ...
            return ft.Column([
                ...
                lista_ass,
                ft.Container(height=20),
                ft.Container(content=ft.Row([t_nome, t_val, ft.IconButton("add_circle", icon_color=COR_PRINCIPAL, on_click=add)]), bgcolor="#1e293b", padding=10, border_radius=10),
                ft.Container(height=50)
            ])
        ...
        def mudar(idx):
            page.clean()
            # Mapeamento do Menu (Importante: 0=Extrato, 1=Ferramentas, 2=Assinaturas)
            # Doar não é uma tela, é uma ação no menu.
