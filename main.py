...
def listar_recentes(limite=5):
    """Pega as ultimas transacoes para a IA saber o que aconteceu"""
...
def gerar_contexto_completo():
    """Cria o cérebro contextual da Autiah"""
    renda = get_renda()
    ...
    return f"""
    VOCÊ É A AUTIAH. Parceira financeira do usuário (neurodivergente).
    CONTEXTO ATUAL:
    - Renda Mensal: {formatar_moeda_visual(renda)}
    - Saldo Total Acumulado: {formatar_moeda_visual(saldo)}
    - Últimas Movimentações:
    {txt_recentes}

    INSTRUÇÕES:
    1. Use esses dados para personalizar a resposta.
    2. Se o saldo for baixo, seja cautelosa.
    3. Seja literal, direta e acolhedora.
    """

# ==============================================================================
# 3. INTERFACE (V52 - ONBOARDING AJUSTADO)
# ==============================================================================
...
def main(page: ft.Page):
    ...
    def tela_onboarding():
        ...
        def acao_pular(e): 
            set_intro_ok()
            # MENSAGEM EXPLÍCITA SOLICITADA
            notificar("Tudo bem! Adicione sua renda depois em Ferramentas.", "#fbbf24") 
            mudar(0)
        ...
        return ft.Container(alignment=ft.alignment.center, content=ft.Column([
            ...
            ft.Text("Comece agora a gerenciar sua renda", size=16, color="grey"), # TEXTO SOLICITADO
    ...
    def tela_ferramentas():
        ...
        box_pix = ft.Container(content=ft.Row([ft.Icon(ft.icons.PIX, color="#32bcad"), ft.Column([ft.Text("Minha Chave Pix", weight="bold"), ft.Text("85996994887", size=12)]), ft.IconButton(icon="content_copy", on_click=copiar_pix)]), bgcolor="#1e293b", padding=10, border_radius=10, border=ft.border.only(left=ft.border.BorderSide(4, "#32bcad")))
